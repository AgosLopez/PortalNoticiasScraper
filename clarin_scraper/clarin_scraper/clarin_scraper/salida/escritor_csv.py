...
#
# IMPORTANTE: este valor es el que aparecerá en el CSV "medios.csv" y en
# la relación PUBLICA de Neo4j. 
# ---------------------------------------------------------------------------
MEDIO_ID = "clarin"
MEDIO_NOMBRE = "Clarín"
...
class EscritorCSV:
    """
    Gestiona la escritura incremental de los datos scrapeados a archivos CSV.

    Por qué CSV y no directo a Neo4j:
      Separar la extracción (scraping) de la carga (import) es una buena
      práctica. Los CSVs actúan como capa intermedia:
        - Si el scraper falla a mitad de camino, no perdiste lo ya extraído.
        - Podés revisar/limpiar los datos antes de cargarlos.
        - Neo4j tiene un comando nativo "LOAD CSV" que los consume directo.

    Estructura de archivos generados (idéntica a Infobae para compatibilidad):
      medios.csv           → nodo Medio
      noticias.csv         → nodo Noticia
      autores.csv          → nodo Autor
      temas.csv            → nodo Tema
      verificaciones.csv   → nodo Verificacion
      rel_publica.csv      → relación (Medio)-[:PUBLICA]->(Noticia)
      rel_escrito_por.csv  → relación (Noticia)-[:ESCRITO_POR]->(Autor)
      rel_menciona.csv     → relación (Noticia)-[:MENCIONA]->(Tema)
      rel_verifica.csv     → relación (Verificacion)-[:VERIFICA]->(Noticia)

    Deduplicación en memoria:
      Guardamos en sets los IDs ya vistos para no repetir nodos en los CSVs.
      Esto es crucial: si scrapeás 100 artículos del mismo autor, ese autor
      debe aparecer UNA sola vez en autores.csv (pero sí 100 veces en
      rel_escrito_por.csv, una por cada relación).
    """

    def __init__(self, directorio_salida: str = "datos") -> None:
        self.directorio = Path(directorio_salida)
        self.directorio.mkdir(parents=True, exist_ok=True)

        # Cargar IDs ya existentes para retomar una ejecución interrumpida
        ...
        self._verificaciones_vistas: set[str] = self._cargar_ids_existentes(
            "verificaciones.csv", "verificacion_id"
        )
        self._inicializar_medios()

    def _cargar_ids_existentes(self, nombre_csv: str, campo_id: str) -> set[str]:
        """Lee un CSV ya existente y devuelve el set de IDs que contiene."""
    ...
    def _csv(self, nombre: str, campos: list[str]):
        """
        Context manager que abre un CSV en modo append (añadir al final).
        Si el archivo no existe todavía, escribe primero la cabecera.

        Por qué append: nos permite reanudar el scraping si se interrumpe,
        sin perder los artículos ya guardados.
        """
    ...
    def _inicializar_medios(self) -> None:
        """Crea medios.csv con la entrada de Clarín si no existe."""
    ...
    def guardar(self, resultado: ResultadoArticulo) -> None:
        """
        Guarda todos los datos de un artículo en los CSVs correspondientes.
        Orden: primero los nodos, después las relaciones.
        """
    ...
    # ------------------------------------------------------------------
    # Métodos internos: un método por entidad/relación
    # ------------------------------------------------------------------
